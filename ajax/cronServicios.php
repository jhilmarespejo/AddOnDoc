...
foreach ($clientes as $cliente) {
    ...
    try {
        $logData = [
            ...
            'numPrestamo' => $cliente->numPrestamo?? 'No definido',
        ...
        if($cod_plan == 'PPAB0049' || $cod_plan == 'PPAB0068' || $cod_plan == 'PPAB0039' || 
            ...
            $ordenVenta = array(
                ...
                "CardCode" => $datosCanal[0]->CardCode,
                "CardName" => $datosCanal[0]->razon_social,
                "NumAtCard" => $contrato, //"CRS-PROVSP-240088",     //----------------
                "DocTotal" => $servicios[0]->precio,   //450.0,   //----------
                "DocCurrency" => "BS",  
                "Comments" => $contrato, //"PPCE0001-LP-24-0001-0000088",   //----------  Generarlo
                "U_NIT" => $datosCanal[0]->NIT,
                "U_RAZSOC" => $datosCanal[0]->razon_social,
                "U_ESTADOFC" => "V",
                "U_COMENTARIOS" => $contrato,   //"PPCE0001-LP-24-0001-0000088|20241205",  //----------
                "U_CANAL" => $canal, //"C003",   //----------
                "U_CONSULTORIO" => $U_CONSULTORIO,
                "U_NumTransaccionExt" => $NumAtCard,  //"5588",  //----------
                "DiscountPercent" => 0.0,
                "TotalDiscount" => 0.0,
    ...
    // // -------------------------------------------------------------- //
        ...
        echo "OV creada para numPrestamo: " . $numPrestamo . "<br> \n";

        $endcont = microtime(true);
        $duration = $endcont - $startcont;

        $texto = " Elemento $tot_cli tomó " . round($duration, 4) . " segundos.<br> \n <br> \n\n";
        // echo $texto;
        ...
        $sql = "UPDATE temps_vit 
                ...
                WHERE numPrestamo = ?";
        $stmt = $connectDB->prepare($sql);
        $stmt->bind_param("ss", $fechaActual, $numPrestamo); // ← corregido aquí
        $stmt->execute();
...
function logSap($logData) {
    ...
    $logDir = __DIR__ . '/logsCron';
    $logFile = $logDir . '/CronServicios.log';
